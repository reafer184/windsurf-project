...
// Simple encryption (XOR-based for client-only, adequate for local threats)
// For production: consider Web Crypto API with PBKDF2
...
const encrypt = (text, password) => {
  const key = deriveKey(password, 'totp-salt-v1');
...
const renderAccounts = async () => {
  ...
  for (const account of accounts) {
    ...
    let code = '••••••';
    try {
      const secret = decrypt(account.secretEnc, state.masterPassword);
    ...
    li.innerHTML = `
      ...
      <div class="code-section">
        ...
        <div class="code-meta">${remaining}s</div>
      </div>
...
const init = async () => {
  ...
  els.addForm?.addEventListener('submit', async (e) => {
    e.preventDefault();
    if (!state.isUnlocked) {
      setStatus('Сначала разблокируйте приложение', 'error');
      return;
    }
    ...
    try {
      ...
      const encrypted = encrypt(secret, state.masterPassword);
  ...
  updateInterval = setInterval(() => {
    if (state.isUnlocked) renderAccounts();
  }, 1000);
};
